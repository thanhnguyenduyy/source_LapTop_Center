			...
			<div class="row">
				<div id="aside" class="col-md-3">
					...
					<div class="aside">
						...
						<ul class="list-links">
							<li><a href="{{ $category->slug }}">Tất cả</a></li>
							@foreach ($category->typeProduct as $item)
								<li @if ($item->name == $typeProduct->name) {{ 'class=active' }} @endif><a href="{{ $category->slug }}/{{ $item->slug }}">{{ $item->name }}</a></li>
							@endforeach
						</ul>
					</div>
					...
					<div class="store-filter clearfix">
						...
						<div class="pull-right">
							...
							<div class="pagination">
								{{ $products->appends(['min' => $min, 'max' => $max, 'sort' => $sort, 'show' => $show])->links() }}
							</div>
